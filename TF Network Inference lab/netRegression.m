...
netLasso[netObject_, predictorFolds_, geneFolds_, lambdas_]:= (
       ...
       AppendTo[netObject["edgeScores"],
       	Normal[
       		Fit[{Join[predictorFolds[[1,1]], predictorFolds[[1,2]]],
       		 	 Join[geneFolds[[1,1]], geneFolds[[1,2]]]},
       		 	 FitRegularization -> {"L1", netObject["lambdasChosen"][[-1]]}
       		 	]
       	]
       ];
      (* Fill in here code that returns the coefficients of the best fit, given the best lambda. To do that, you'll 
         ...
         best lambda
         1. You can get the complete predictor set by combining the training and test components of the first
         predictor fold (or any predictor fold). Likewise you can get the complete response set by combining the two
         parts of any gene fold.
         2. You can pull the best lambda back from the netObject with netObject["lambdasChosen"][[-1]]. *) 
...
constructErrorsMatrix[predictorFolds_, geneFolds_, lambdas_]:=  Module[{errorsMatrixLambdasByFolds,lambdaIndex},
  (* Initialize an empty matrix to store errors for each lambda and fold *)
  errorsMatrixLambdasByFolds = ConstantArray[0, {Length[lambdas], Length[geneFolds]}];
  Do[errorsMatrixLambdasByFolds[[lambdaIndex]] = Map[sumSquaredTestError[predictorFolds[[#]],geneFolds[[#]],lambdas[[lambdaIndex]]]&,
				Range[Length[predictorFolds]]]
				,{lambdaIndex,Length[lambdas]}
	];
	errorsMatrixLambdasByFolds
]
...
(* chooseLambda takes an errors matrix and computes the total error across folds for each lambda and then choses the 
   lambda whose error is least. Write the body of this function below.
*)
chooseLambda[errorsMatrixLambdasByFolds_, lambdas_]:= Module[{totalErrors, bestLambdaIndex, bestLambda},
  (* Calculate total errors across folds for each lambda *)
  totalErrors = Total[errorsMatrixLambdasByFolds, {2}];
  (* Find the index of the lambda with the least total error *)
  bestLambdaIndex = First@Ordering[totalErrors];
  bestLambda = lambdas[[bestLambdaIndex]];
  bestLambda
]
  
     
(* presentNetAccuracy creates a precision-vs-rank plot and a precision-recall plot, with a line indicating
...
scoreNet[netObject_, bindingMatrixPath_] :=
 ...
 (* evalEdges takes the list of ranked edges with non-zero scores that constructEdgeList outputs (see tools.m).
    ...
    *)     	 
    (* Write the body code for evalEdges. *)
evalEdges[rankedEdges_, bindingMatrix_]:= Module[{numPossibleEdges, truePositives = 0, i, precisionRecallList},
  (* Calculate the number of possible edges supported by binding data *)
  numPossibleEdges = Total[Flatten[bindingMatrix]];
  precisionRecallList = {};
  For[i = 1, i <= Length[rankedEdges], i++,
    (* Check if the edge is supported by binding data *)
    If[bindingMatrix[[rankedEdges[[i, 2]], rankedEdges[[i, 1]]]] == 1, truePositives++; 
    ];
    (* Calculate precision and recall and append to the list *)
    AppendTo[precisionRecallList, {i, truePositives/i, truePositives/numPossibleEdges}];
  ];
  precisionRecallList
]
